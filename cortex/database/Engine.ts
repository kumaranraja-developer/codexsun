...
export interface Engine {
    ...
    readonly driver: 'mariadb' | 'postgres' | 'sqlite';
...
export abstract class BaseEngine implements Engine {
    ...
    public readonly driver: 'mariadb' | 'postgres' | 'sqlite';
    ...
    async connect()           { await this._connect(); }
    async close()             { await this._close(); }
    async testConnection()    { return this._test_connection(); }
    getConnection()           { return this._get_connection(); }

    async execute(sql: string, params?: unknown)      { return this._execute(sql, params); }
    async fetchOne<T>(sql: string, params?: unknown)  { return this._fetchone<T>(sql, params); }
    async fetchAll<T>(sql: string, params?: unknown)  { return this._fetchall<T>(sql, params); }
    async executeMany(sql: string, paramSets: unknown[]) { return this._executemany(sql, paramSets); }

    async begin()   { await this._begin(); }
    async commit()  { await this._commit(); }
    async rollback(){ await this._rollback(); }
