...
export async function smokeMariaDB(): Promise<void> {
    log.header('[smoke:mariadb]');
    const eng = await prepareEngine(PROFILE);
    if (eng.driver !== 'mariadb') {
        throw new Error(`Profile ${PROFILE} is ${eng.driver}, expected mariadb`);
    }
    const tbl = makeTable();
    const create = `
    CREATE TABLE ${tbl} (
  ...
  `;
    ...
    try {
        ...
        log.ok(`Inserted & verified ${TOTAL} rows in MariaDB (${color.gray(tbl)})`);
    } finally {
